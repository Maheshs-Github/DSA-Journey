...
// So what is the two sum if have given some numbers , which of 2 no sum is equal to given no 
// Revision
...
public class TwoSum{
  ...
  // optimal , but only care about the values
  // public int[] sum(int[] a,int k){
  //   Arrays.sort(a);
  ...
  // optimal while preserving the order
  // public int[] sum(int[] a,int k){
  //   HashSet<Integer> h=new HashSet<>();
  //   for (int i = 0; i < a.length; i++) {
  //     int ans=k-a[i];
  //     System.out.println("Ans: "+ans);
  //     if(h.contains(ans))
  //       return new int[] {a[i],ans};
  //     h.add(a[i]);
  //   }
  //   return new int[]{-1,-1};
  // }

  // well we have to write the index
    public int[] sum(int[] a,int k){
    HashMap<Integer,Integer> h=new HashMap<>();
    for (int i = 0; i < a.length; i++) {
      int ans=k-a[i];
